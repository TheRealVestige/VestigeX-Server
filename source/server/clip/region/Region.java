...
public class Region {
        ...
        for(int ii = 0; ii < max; ii++)
        {
            ...
            for(int i = 0; i < xLength; i++)
            {
                for(int i2 = 0; i2 < yLength; i2++)
                    ...
                    if(diffX == 0 && diffY < 0 && (getClipping(currentX + i, (currentY + i2) - 1, height) & 0x1280102) != 0)
                        return false;
    ...
    public static void load() {
        ...
        for (int i = 0; i < size; i++) {
            byte[] file1 = ReadFile(("./Data/data/map/" + mapObjectsFileIds[i] + ".dat"));
		...
		System.out.println("[Server] Finished Loading Maps");
		System.out.println("[Server] Vestige-x is Now Online!");
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    }
	...
	public static boolean canAttack(NPC npc, Client p) {
		return canMove(npc.getX(), npc.getY(), p.getX(), p.getY(), npc.heightLevel % 4, 1, 1);
	}
