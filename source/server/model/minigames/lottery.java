...
public class lottery {
	...
	public int prizeAmount = 250; //jackpot amount (in millions)
	...
	public void enterLottery(Client c) {
		if (checkEntriesCount(c) < maximumEntryTimes) {
			if (c.getItems().playerHasItem(995, entryPrice * 1000000)) {
				...
				c.sendMessage("You have been entered into the lottery, when the lottery fund reaches 250m a winner");
	...
	public void announceFund() {
		...
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				Client all = (Client)Server.playerHandler.players[j];
				all.sendMessage("<shad=40960>[Lottery]<shad=-1> The Lottery Fund is currently at " + fund + "m. Enter by talking to lottie at Edgeville");
			}
		}
	}
