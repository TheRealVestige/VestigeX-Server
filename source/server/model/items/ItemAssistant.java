					...
					if(targetSlot == 10 || targetSlot == 7 || targetSlot == 5 || targetSlot == 4 || targetSlot == 0 || targetSlot == 9 || targetSlot == 10) {
						...
						if(c.rangeLevelReq > 0) {
							...
							if(c.getPA().getLevelForXP(c.playerXP[24]) < c.dungLevelReq) {
								c.sendMessage("You need a Dungeoneering level of "+c.dungLevelReq+" to wear this item.");
								canWearItem = false;
							}
						}
...
return false;
		...
		c.getPA().sendFrame126(bankSlotsUsed()+" / "+Config.BANK_SIZE, 23003);
			...
			c.sendMessage("AntiDupe: You tried to bank an item while dueling, duel closed.");
                             		return false;
                        	}*/
		if(c.inTrade) {
			...
			return false;
						...
						if (c.lastAttackedBy != null) {
						c.sendMessage("AntiDupe: Cannot bank item, errorcode: You can't drop items in combat.");
						return false;
				}*/
	...
	**/
	...
	public void dropArrowNpc() {
		...
		/*if (c.playerEquipment[c.playerWeapon] == 13883) // MORRIGAN'S AXE
			return;
		if (c.playerEquipment[c.playerWeapon] == 13879) // MORRIGAN'S JAVELIN
			return;*/
