...
public class ShopAssistant {
	...
	**/
	...
	public void buyFromShopPrice(int removeId, int removeSlot){
		...
		if (c.myShopId == 20) {
			c.sendMessage(c.getItems().getItemName(removeId)+": currently costs " + getSpecialItemValue(removeId) + " Vestige-x Points!");
			return;
		}
		if (c.myShopId == 18) {
			c.sendMessage(c.getItems().getItemName(removeId)+": currently costs " + getSpecialItemValue(removeId) + " Vestige-x Points!");
		...
		if (c.myShopId == 97) {
			c.sendMessage(c.getItems().getItemName(removeId)+": currently costs " + getSpecialItemValue(removeId) + " Custom Points.");
			return;
		}
		if (c.myShopId == 72) {
		...
		if (c.myShopId == 77) {
			c.sendMessage(c.getItems().getItemName(removeId)+": currently costs " + getSpecialItemValue(removeId) + " Loyalty Points.");
			return;
		}
		if (c.myShopId == 95) {
			c.sendMessage(c.getItems().getItemName(removeId)+": currently costs " + getSpecialItemValue(removeId) + " Achievement Points.");
			return;
		}
		if (c.myShopId == 16) {
        ...
        if (c.myShopId == 43) {
                c.sendMessage(c.getItems().getItemName(removeId)+": currently costs " + getSpecialItemValue(removeId) + " Dominion Tower Points.");
                return;
        }
		if (c.myShopId == 100) {
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0) {
			...
			for (int i = amount; i > 0; i--) {
				...
				if (Slot == -1 && c.myShopId != 11 && c.myShopId != 29 && c.myShopId != 30 && c.myShopId != 31 && c.myShopId != 95 && c.myShopId != 72 && c.myShopId != 77 && c.myShopId != 84 && c.myShopId != 85 && c.myShopId != 100) {
			...
			return false;
                	...
                	if (c.LoyaltyPoints >= TotPrice2) {
						if (c.getItems().freeSlots() > 0) {
							buyDelay = System.currentTimeMillis();
							c.LoyaltyPoints -= TotPrice2;
							c.getItems().addItem(itemID, 1);
							Server.shopHandler.ShopItemsN[c.myShopId][fromSlot] -= 1;
							Server.shopHandler.ShopItemsDelay[c.myShopId][fromSlot] = 0;
							if ((fromSlot + 1) > Server.shopHandler.ShopItemsStandard[c.myShopId]) {
                ...
                else if(c.myShopId != 11 && c.myShopId != 29 || c.myShopId != 30 || c.myShopId != 31 || c.myShopId != 84  || c.myShopId != 95 || c.myShopId != 97 || c.myShopId != 77 || c.myShopId != 72 || c.myShopId != 85 || c.myShopId != 16) {
		...
		public void handleOtherShop(int itemID) {
			if (c.myShopId == 20) {
				if (c.pcPoints >= getSpecialItemValue(itemID)) {
					...
					c.sendMessage("You do not have enough Vestige-x Points to buy this item.");			
				}
