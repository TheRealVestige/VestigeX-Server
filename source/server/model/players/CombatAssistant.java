...
public class CombatAssistant{
	...
	*/
	...
	if (c.playerEquipment[c.playerWeapon] == 13347) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 7783) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 19939) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 19931) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 13196) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 11144) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 11148) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 20135) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 20120) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 20130) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 20155) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 19477) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 4640) {
	c.doubleHit = true;
	}
	...
	if(c.playerEquipment[c.playerWeapon] == 15241 && 15243 != c.playerEquipment[c.playerArrows]){
                ...
                return;
		  } // here starts msb fixes
	if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9245){
                c.sendMessage("You cant use bolts with mage shortbow!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9143){
                c.sendMessage("You cant use bolts with mage shortbow!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9142){
                c.sendMessage("You cant use bolts with mage shortbow!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9141){
                c.sendMessage("You cant use bolts with mage shortbow!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9140){
                c.sendMessage("You cant use bolts with mage shortbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9244){
                c.sendMessage("You cant use bolts with mage shortbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9144){
                c.sendMessage("You cant use bolts with mage shortbow!");
                return;
	} // made it here..
			if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9144){
		...
		if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 892){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 892){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 15243){
                c.sendMessage("You can't fire hand cannon shots with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 20155 && c.playerEquipment[c.playerArrows] == 892){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 20155 && c.playerEquipment[c.playerArrows] == 15243){
                c.sendMessage("You can't fire hand cannon shots with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 19477 && c.playerEquipment[c.playerArrows] == 892){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 19477 && c.playerEquipment[c.playerArrows] == 15243){
                c.sendMessage("You can't fire hand cannon shots with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 4640 && c.playerEquipment[c.playerArrows] == 892){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 4640 && c.playerEquipment[c.playerArrows] == 15243){
                c.sendMessage("You can't fire hand cannon shots with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 11235 && c.playerEquipment[c.playerArrows] == 9144){
                c.sendMessage("You can't use bolts with a bow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 11235 && c.playerEquipment[c.playerArrows] == 9245){
                c.sendMessage("You can't use bolts with a bow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 11235 && c.playerEquipment[c.playerArrows] == 9244){
                c.sendMessage("You can't use bolts with a bow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 11235 && c.playerEquipment[c.playerArrows] == 9243){
                c.sendMessage("You can't use bolts with a bow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 4740){
                c.sendMessage("You can't use bolt racks with chaotic crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9244){
                c.sendMessage("You can't use bolts with a magic shortbow broski!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9243){
                c.sendMessage("You can't use bolts with a magic shortbow broski!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 15243){
                c.sendMessage("You cant fire hand cannon shots without the hand cannon!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 15243){
                c.sendMessage("You cant fire hand cannon shots without the hand cannon!");
		...
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 9245){
                c.sendMessage("Only Chaotic Crossbow has the strength to fire these powerful bolts!");
                return;
					}
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 11212){
                c.sendMessage("You can't fire D Arrows with Crossbows!");
                return;
									}
		if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 11212){
                c.sendMessage("You can't fire D Arrows with Crossbows!");
                return;
	} // HERE STARTS RUNE CROSS WITH ARROWS / GABBE CHEAP FIX
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 890){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
					}
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 888){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
					}
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 886){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
					}
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 884){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
		if(c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerArrows] == 882){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	} // here starts karils crossbow cheap fix
			if(c.playerEquipment[c.playerWeapon] == 4734 && c.playerEquipment[c.playerArrows] == 11212){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 4734 && c.playerEquipment[c.playerArrows] == 892){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 4734 && c.playerEquipment[c.playerArrows] == 890){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
			if(c.playerEquipment[c.playerWeapon] == 4734 && c.playerEquipment[c.playerArrows] == 9244){
                c.sendMessage("You can't fire bolts with this crossbow, only bolt rack!");
                return;
	}
				if(c.playerEquipment[c.playerWeapon] == 4734 && c.playerEquipment[c.playerArrows] == 9245){
                c.sendMessage("You can't fire bolts with this crossbow, only bolt rack!");
                return;
	}
				if(c.playerEquipment[c.playerWeapon] == 4734 && c.playerEquipment[c.playerArrows] == 9144){
                c.sendMessage("You can't fire bolts with this crossbow, only bolt rack!");
                return;
	}
				if(c.playerEquipment[c.playerWeapon] == 4734 && c.playerEquipment[c.playerArrows] == 9243){
                c.sendMessage("You can't fire bolts with this crossbow, only bolt rack!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 890){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
					}
		if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 888){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
					}
		if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 886){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
					}
		if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 884){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
									}
		if(c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerArrows] == 882){
                c.sendMessage("You can't fire arrows with a crossbow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 4734 && 4740 != c.playerEquipment[c.playerArrows]){
c.sendMessage("You need Bolt Rack for Karil's Crossbow!");
return;
}
...
//				c.sendMessage("This is "+Server.npcHandler.npcz[i]+" NPC.");
			...
			if (Server.npcHandler.npcs[i].underAttackBy > 0 && Server.npcHandler.npcs[i].underAttackBy != c.playerId && !Server.npcHandler.npcs[i].inMulti()) {
				c.npcIndex = 0;
				c.sendMessage("This NPC is already in combat.");
				return;
			}
			if (Server.npcHandler.npcs[i].inMulti() && c.hasFollower > 0) {
				Server.npcHandler.attackNPC(i, c.summoningnpcid);
			}
			if ((c.underAttackBy > 0 || c.underAttackBy2 > 0) && c.underAttackBy2 != i && !c.inMulti() && !c.inBarbDef) {
				resetPlayerAttack();
			...
			if(c.attackTimer <= 0) {
				...
				boolean usingCross = c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerWeapon] == 20155 && c.playerEquipment[c.playerWeapon] == 4640 && c.playerEquipment[c.playerWeapon] == 19477 && c.playerEquipment[c.playerWeapon] == 18357;
				c.bonusAttack = 0;
				c.rangeItemUsed = 0;
				c.projectileStage = 0;
				//c.SaveGame();
				...
				if(!usingCross && !usingArrows && usingBow && !(c.playerEquipment[c.playerWeapon] == 9185 && (c.playerEquipment[c.playerWeapon] < 4212 || c.playerEquipment[c.playerWeapon] > 4223))) {
					c.sendMessage("You have run out of arrows!");
					c.stopMovement();
					c.npcIndex = 0;
					return;
				} 
				if(!usingCross && !usingArrows && usingBow && !(c.playerEquipment[c.playerWeapon] == 20155 && (c.playerEquipment[c.playerWeapon] < 4212 || c.playerEquipment[c.playerWeapon] > 4223))) {
					c.sendMessage("You have run out of arrows!");
					c.stopMovement();
					c.npcIndex = 0;
					return;
				...
				if (usingCross && !properBolts() && !properGabbe()) {
			...
			c.followId2 = i;
				...
				if(c.usingMagic) {	// magic hit delay
					...
					c.SSPLIT = true;
if(c.curseActive[18] && c.oldPlayerIndex > 0) { // SoulSplit SETUP
c2.gfx0(2264);
	...
	public void delayedHit(int i) { // npc hit delay
		if (Server.npcHandler.npcs[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				}if (Misc.random(5) == 1 && c.lastArrowUsed == 9243 && c.playerEquipment[c.playerWeapon] == 20155 && c.playerEquipment[c.playerWeapon] == 19477 && c.playerEquipment[c.playerWeapon] == 4640 && c.playerEquipment[c.playerWeapon] == 18357) {
					ignoreDef = true;
					Server.npcHandler.npcs[i].gfx0(758);
				}
				...
				if (Misc.random(5) == 1 && c.lastArrowUsed == 9242 && damage > 0 && c.playerEquipment[c.playerWeapon] == 20155) {
					Server.npcHandler.npcs[i].gfx0(754);
					damage = Server.npcHandler.npcs[i].HP/5;
					//c.handleHitMask(c.playerLevel[3]/10);
					c.dealDamage(c.playerLevel[3]/10);
				...
				if (Misc.random(5) == 1 && c.lastArrowUsed == 9242 && damage > 0 && c.playerEquipment[c.playerWeapon] == 19477) {
					Server.npcHandler.npcs[i].gfx0(754);
					damage = Server.npcHandler.npcs[i].HP/5;
					//c.handleHitMask(c.playerLevel[3]/10);
					c.dealDamage(c.playerLevel[3]/10);
				...
				if (Misc.random(5) == 1 && c.lastArrowUsed == 9242 && damage > 0 && c.playerEquipment[c.playerWeapon] == 4640) {
					Server.npcHandler.npcs[i].gfx0(754);
					damage = Server.npcHandler.npcs[i].HP/5;
					//c.handleHitMask(c.playerLevel[3]/10);
					c.dealDamage(c.playerLevel[3]/10);
				...
				if (damage > 0 && Misc.random(5) == 1 && c.lastArrowUsed == 9244 && c.playerEquipment[c.playerWeapon] == 20155) {
					damage *= 1.45;
					Server.npcHandler.npcs[i].gfx0(756);
				}
				if (damage > 0 && Misc.random(5) == 1 && c.lastArrowUsed == 9244 && c.playerEquipment[c.playerWeapon] == 19477) {
					damage *= 1.45;
					Server.npcHandler.npcs[i].gfx0(756);
				}
				if (damage > 0 && Misc.random(5) == 1 && c.lastArrowUsed == 9244 && c.playerEquipment[c.playerWeapon] == 4640) {
					damage *= 1.45;
					Server.npcHandler.npcs[i].gfx0(756);
				}
				...
				//damage = (int)(damage * c.getPA().shieldEffect(c, 2));
...
if(c.playerEquipment[c.playerWeapon] == 11235 && 11212 != c.playerEquipment[c.playerArrows]){
c.sendMessage("You need dragon arrows to attack with a dark bow");
return;
}
if(c.playerEquipment[c.playerWeapon] == 4734 && 4740 != c.playerEquipment[c.playerArrows]){
c.sendMessage("You need Bolt Rack for Karil's Crossbow!");
return;
		...
		if (Server.playerHandler.players[i] != null) {
			...
			if(c.attackTimer <= 0) {
				...
				if(!usingCross && !usingArrows && usingBow && !(c.playerEquipment[c.playerWeapon] == 9185 || (c.playerEquipment[c.playerWeapon] < 4212 || c.playerEquipment[c.playerWeapon] > 4223) && !c.usingMagic)) {
					c.sendMessage("You have run out of arrows!");
					c.stopMovement();
					resetPlayerAttack();
					return;
				}
				if(correctBowAndArrows() < c.playerEquipment[c.playerArrows] && Config.CORRECT_ARROWS && usingBow && !usingCrystalBow() && usingCross && !c.usingMagic) {
					c.sendMessage("You can't use "+c.getItems().getItemName(c.playerEquipment[c.playerArrows]).toLowerCase()+"s with a "+c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase()+".");
					c.stopMovement();
					resetPlayerAttack();
					return;
	...
	if(!usingCross && !usingArrows && usingBow && !(c.playerEquipment[c.playerWeapon] == 9185 && (c.playerEquipment[c.playerWeapon] < 4212 || c.playerEquipment[c.playerWeapon] > 4223))) {
				...
				if (usingCross && !properGabbe() && !properBolts() && !c.usingMagic) {
					c.sendMessage("You must use bolts with a crossbow.");
					c.stopMovement();
					resetPlayerAttack();
					return;				
	...
	public void playerDelayedHit(int i) {
		if (Server.playerHandler.players[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				//damage = (int)(damage * c.getPA().shieldEffect(o, 1));
				...
				if (Misc.random(5) == 1 && c.lastArrowUsed == 9243 && c.playerEquipment[c.playerWeapon] == 20155) {
					ignoreDef = true;
					o.gfx0(758);
				}	
				if (Misc.random(5) == 1 && c.lastArrowUsed == 9243 && c.playerEquipment[c.playerWeapon] == 19477) {
					ignoreDef = true;
					o.gfx0(758);
				}	
				if (Misc.random(5) == 1 && c.lastArrowUsed == 9243 && c.playerEquipment[c.playerWeapon] == 4640) {
					ignoreDef = true;
					o.gfx0(758);
				}	
		...
		case 13346: // Virtus Legs
			damage *= .04;
				...
				Server.playerHandler.players[i].handleHitMask(damage, Server.playerHandler.players[i].playerId);
...
Server.playerHandler.players[i].handleHitMask(damage2, Server.playerHandler.players[i].playerId);
		...
		if (Server.playerHandler.players[playerId] != null) {
			...
			if (checkMultiBarrageReqs(playerId)) {
				...
				if (Misc.random(mageAtk()) > Misc.random(mageDef()) && !c.magicFailed) {
					...
					Server.playerHandler.players[playerId].handleHitMask(damage, Server.playerHandler.players[playerId].playerId);
...
//		c.PlayerAttType = 2;
	...
	public boolean checkNpcReq(int i) {
		...
		if (NPCHandler.npcs[i].spawnedBy != c.playerId && NPCHandler.npcs[i].spawnedBy > 0 && !c.InDung) {
			resetPlayerAttack();
			c.sendMessage("This monster was not spawned for you.");
			return false;
		}
...
break;
			...
			/*	case 19780:
				...
				case 19780:
       c.gfx0(1224); c.startAnimation(1872); if
      (c.playerIndex > 0) { Client opp = (Client)
      Server.playerHandler.players[c.playerIndex]; if(opp != null)
      opp.gfx0(1224); } else if (c.npcIndex > 0) { NPC opp =
      Server.npcHandler.npcs[c.npcIndex]; if(opp != null) opp.gfx0(1224); }
...
case 11142:
	...
	**/
	...
	public boolean checkReqs() {
		...
		if(!Server.playerHandler.players[c.playerIndex].inWild() && !Server.playerHandler.players[c.playerIndex].inPits() && !Server.playerHandler.players[c.playerIndex].inDuelArena()) {
			c.Snowball(c);
			c.stopMovement();
			c.getCombat().resetPlayerAttack();
			return false;
		}
		if(!Server.playerHandler.players[c.playerIndex].inWild() && !Server.playerHandler.players[c.playerIndex].InDung() && !Server.playerHandler.players[c.playerIndex].inDuelArena() && Server.playerHandler.players[c.playerIndex].safeTimer <= 0 && !Server.playerHandler.players[c.playerIndex].inPits()) {
		...
		if(!c.inWild() && c.safeTimer <= 0) {
        	...
        	if(c.inDrive()) {
			c.sendMessage("You are Driving in a safe Zone!");
			c.stopMovement();
			c.getCombat().resetPlayerAttack();
			return false;
		}
 ...
 if(weaponName.contains("2h sword") || weaponName.contains("godsword") || weaponName.contains("saradomin sw")) {
	...
	*/
	public int getBlockEmote() {
		...
		if (c.playerEquipment[c.playerShield] >= 20072 && c.playerEquipment[c.playerShield] <= 20073) {
			return 4177;
		}
		switch(c.playerEquipment[c.playerWeapon]) {
	...
	**/
	...
	public int rangeMaxHit() {
	    ...
	    rangehit += c.playerLevel[4] / 17;
			...
			rangehit = 6;
            rangehit += c.playerLevel[4] / 5;
        } else if (Arrows == 9245 && weapon == 18357) {//c crossbow
			rangehit = 7;
	...
	public boolean properGabbe() {
	return c.playerEquipment[c.playerArrows] >= 9245;
	}
	...
	**/
	...
	public boolean checkMagicReqs(int spell) {
		...
		if(c.usingMagic && Config.RUNES_REQUIRED) {
			...
			if (c.playerEquipment[c.playerWeapon] == 19476 && Misc.random(8) == 0)  {
...
try {
       ...
       Server.getTaskScheduler().addEvent(new Task(2, false) {
		public void execute() {
			...
			c.sendMessage("You must wait another <shad=15007744>"+c.dfsCounter+"</shad> seconds before using this again.");
			}				
			} else {
...
*/
	...
	public void handleSOLeffect() {
			if (c.SOL_PROTECTION == 0) {
				c.SOL_PROTECTION = 100;
       ...
       Server.getTaskScheduler().addEvent(new Task(1, false) {
		public void execute() {
					...
					if (c.SOL_PROTECTION == 0 || c.playerEquipment[c.playerWeapon] != 15486) {
						c.SOL_PROTECTION = 0;
						this.stop();
					}
				}
		...
		public void decreaseSOL() {
			c.SOL_PROTECTION--;
		}
		
		public int getSOLeffect(int damage,int i) {
			Client o = (Client)PlayerHandler.players[i];
			if (o.playerEquipment[c.playerWeapon] == 15486 && o.SOL_PROTECTION >= 1) {
				double effect = 0;
		 		effect = (damage * .5);
				o.gfx0(2319);
				damage = (int)effect;
