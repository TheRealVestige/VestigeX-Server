...
public boolean ppsRefresh = false;
	...
	public String lastKilled = "";
		private Firemaking firemaking = new Firemaking(this);
                                           private Summonin summonin = new Summonin(this);
	...
	public void ppsDegrade() {
		if (playerEquipment[playerWeapon] == 19476 && ppsLeft < 1) {
			playerEquipment[playerWeapon] = -1;
			playerEquipmentN[playerWeapon] = 0;
			getItems().wearItem(-1, 1, 3);
			getItems().addItem(19476, 1);
			autocasting = false;
			usingMagic = false;
			autocastId = -1;
			//ppsLeft = 10;
			sendMessage("Your Polypore staff has no more charges and degrades! ");
			//ppsMagicAuto = false;
			//ca.resetPlayerAttack();
		}
	}
        ...
        for (int j = 0; j < Server.playerHandler.players.length; j++) {
            if (Server.playerHandler.players[j] != null) {
                Client c2 = (Client) Server.playerHandler.players[j];
                    //c2.sendMessage("<shad=7>"+playerName+" has just completed the Dominion Tower challenge.");
                }
            }
        }
...
***/
	...
	int[] DragonImpRare = {829,391,385,564,1249,3052,1303,808,1602,1917,1971,1514,1319,1275,3024,1606,454,560,563,561,1373,1127,5300,2440,1604,1452,1149,830,1079,886,1608,565,554,1079,1093,1127,1432,1333,1185,2366,1249,1303,385,3024,564,3052,561,1275,5300,989,19337,19341,19336,19338,19339,19340,19342,19343,19345,19344};
        public int DragonImpRare() {
                        return DragonImpRare[(int) (Math.random() * DragonImpRare.length)];
                        }
int[] DragonImpCommon = {829,391,385,564,1249,3052,1303,808,1602,1917,1971,1514,1319,1275,3024,1606,454,560,563,561,1373,1127,5300,2440,1604,1452,1149,830,1079,886,1608,565,554,1079,1093,1127,1432,1333,1185,2366,1249,1303,385,3024,564,3052,561,1275,5300};
        public int DragonImpCommon() {
                        return DragonImpCommon[(int) (Math.random() * DragonImpCommon.length)];
                        }
int[] DragonImpBones = {526};
        public int DragonImpBones() {
                        return DragonImpBones[(int) (Math.random() * DragonImpBones.length)];
}
	
	public void Lootimpjar(int itemId, int playerId) {
if (itemId == 11256){    
        if (Misc.random(1000) <= (1 + Misc.random(ImpLuck))) {
                ...
                if (Misc.random(250) <= (10 + Misc.random(ImpLuck))) {
                        getItems().addItem(DragonImpCommon(), 1);
                        getItems().deleteItem(11256, 1);
                        sendMessage("You open the jar and find a <col=1532693>Common</col>item");
                        ImpLuck -= 3;
                }else{
                        getItems().addItem(DragonImpBones(), 1);
                        getItems().deleteItem(11256, 1);
                        sendMessage("You open the jar and find some bones");
                        ImpLuck += 1;
                        }
                }
        }
...
vlsLeft = 1000;
	...
	public void turnIntoVLS() {
		if(playerEquipment[playerWeapon] != 13899) {
		return;
		}
	if(playerEquipment[playerWeapon] == 13899) {
	...
	if(playerEquipment[playerWeapon] == 13899 && vlsLeft2 == 0) {
	sendMessage("Your Vesta longsword has turned into a (Deg) Wep!!");
	playerEquipment[playerWeapon] = 13901;
	getItems().wearItem(13901, 1, 3);
	vlsLeft2 = 500;
	}
						if(vlsLeft2 < 1) {
						vlsLeft2 = 500;
	...
	public void DegradeVLS() {
			if(playerEquipment[playerWeapon] != 13901) {
		return;
		}
		if(playerEquipment[playerWeapon] == 13901) {
		degWep = true;
	...
	public void Hack3rsAutoSaver() {
	if(!disconnected) {
	System.out.println("Started saving accounts");
	   EventManager.getSingleton().addEvent(new Event() {
		public void execute(EventContainer container) { 
sendMessage("<shad=132833>Vestige-x: Game saved successfully for everyone!");
...
Server.shutdown();
System.out.println("SERVER SHUT DOWN BY EVENT shutDown();");
...
/*	public void dungRest() {
  if(dungRest < 0 || dungRest > 0) {
       Server.getTaskScheduler().addEvent(new Task(1, false) {
		public void execute() {
	                    dungRest --;
									if(disconnected) {
		...
		return;
					...
					startAnimation(896);
					getItems().deleteItem(1779, getItems().getItemSlot(1779), 1);
...
public void HighAndLow(){
	...
	public void crackcracker() {
		if (getItems().playerHasItem(962)) {
		getItems().deleteItem(962, 1);
		getItems().addItem(randomcracker(), 1);
	}
	else
		sendMessage("You need 2 free slots.");
	}
	public static int randomcracker[] = {1038, 1040, 1042, 1044, 1046, 1048, 1050, 1037, 4565};
	public static int randomcracker() {
	return randomcracker[(int) (Math.random() * randomcracker.length)];
	}
...
npcName = Server.npcHandler.getNpcListName(npcId);
	if (System.currentTimeMillis() - foodDelay >= 1500) { //anti spamm
		if (playerLevel[22] >= Req && getItems().playerHasItem(11260, 1)) { //first we check if he's high enough to catch
			if (playerEquipment[playerWeapon] == 10010 || playerEquipment[playerWeapon] == 11259) { //player got net?
				...
				sendMessage("You Caught an Impling!"); //looks like player got a net
		...
		sendMessage("Make sure you've got an impling jar!");
		return;
		}
			...
			if (playerLevel[22] >= Req && getItems().playerHasItem(11260, 1)) { // first we check if he's high enough
				...
				if (playerEquipment[playerWeapon] == 10010
					...
					if (playerLevel[22] + Misc.random(10) >= Misc.random(20)
							+ Req) { // catch chance
						if (Misc.random(1000) == 1) {
							sendMessage("You caught a GIGANTIC butterfly and gained triple Experience!"); 
							getItems().addItem(722, 1); 
							getItems().deleteItem(11260, 1);
							startAnimation(6999); 
							getPA().addSkillXP(AmtExp * 3, 22); 
						} else {
							sendMessage("You caught a Butterfly!"); 
					...
					sendMessage("You need to wear a butterfly net in order to hunt!");
					return;
				}
				} else {
	...
	public void restartDuelIntsAndBooleans() {
		...
		tradeResetNeeded = false;
	        ...
	        Cons.SpawnDaAltar(this);
	        Cons.SpawnDaBed(this);
	        Cons.SpawnDaTree(this);
	        Cons.SpawnDaMuneyNiggaChest(this);
	        Cons.SpawnLectaNiggas(this);
	        Cons.SpawnTeleporter(this);
	        Cons.SpawnCrystal(this);
	}
	 public boolean checkVotes(String playerName) {
                try {
                        String urlString = "http://vestigex.com/vote.php?type=checkvote&username="+playerName;
...
//sendMessage("Alert##Welcome to "+Config.SERVER_NAME+"!!##100 Loyalty Points/HOUR##Bugs/Features Fixed!");
			...
			setSidebarInterface(1, 16700);//Player Tab
...
setSidebarInterface(13, 17011); //music tab 6299 for lowdetail. 962 for highdetail
...
SQL.destroyConnection();
	...
	public void SaveGame() {
        ...
        public void t2p() {
		RequestHelp.teleportToPlayer(this);
	}
	...
	public void process() {
                getPA().sendFrame126(""+LoyaltyPoints+" Loyalty Points", 12226);
                getPA().sendFrame126("@lre@"+Config.SERVER_NAME+" Points: @gre@"+pcPoints+" ",16432);
		getPA().sendFrame126("@lre@Dungeoneering Pts: @gre@"+dungPoints +" ",16433);
		getPA().sendFrame126("@lre@Loyalty Points: @gre@"+LoyaltyPoints+" ",16434);
		getPA().sendFrame126("@lre@Custom Points: @gre@"+customPoints+" ",16435);
		getPA().sendFrame126("@lre@Achievement: @gre@"+achievementPoints+"",16440);
		getPA().sendFrame126("                 @cya@Hello "+playerName+"!", 18217);
	        getPA().sendFrame126("                              @cya@"+PlayerHandler.getPlayerCount()+" Players Online!", 18218);
	        //PPANEL
	        getPA().sendFrame126("@or2@Task: @gre@" + taskAmount + " " + Server.npcHandler.getNpcListName(slayerTask) + "", 16338);
		if (xpLock == false)
                getPA().sendFrame126("@lre@Your Xp is: @gre@UNLOCKED", 16336);
                else if (xpLock == true)
                getPA().sendFrame126("@lre@Your Xp is: @gre@LOCKED", 16336);
	        //END OF PPANEL
	...
	if(ClickItem.flowerTime > 0) {
			ClickItem.flowerTime --;
                     ...
                     LoyaltyScore += 1;
                        ...
                        if(LoyaltyScore == 200) {
			sendMessage("<shad=15695415>[Loyalty]</col>You will Receive 100 Loyalty Points in 1 Hour!");
			sendMessage("<shad=155415>[Server]</col>Don't Forget To Register on the ::forums");
			}
                                                                if(LoyaltyScore == 1800) {
			sendMessage("<shad=15695415>[Loyalty]</col>You will Receive 100 Loyalty Points in 30 Mins!");
			sendMessage("<shad=155415>[Server]</col>Don't Forget To Register on the ::forums");
			}
			if(LoyaltyScore == 6000 || LoyaltyScore > 6200) {
			LoyaltyPoints += 100;
			
			LoyaltyScore = 0;
		sendMessage("<shad=15695415>[SERVER]</col>You have just been awarded 100 Loyalty Points!");
		achievement[8]+= 1;
		achievement[7]+= 1;
			}
...
/*	int totalz = (getLevelForXP(playerXP[0]) + getLevelForXP(playerXP[1]) + getLevelForXP(playerXP[2]) + getLevelForXP(playerXP[3]) + getLevelForXP(playerXP[4]) + getLevelForXP(playerXP[5]) + getLevelForXP(playerXP[6]) + getLevelForXP(playerXP[7]) + getLevelForXP(playerXP[8]) + getLevelForXP(playerXP[9]) + getLevelForXP(playerXP[10]) + getLevelForXP(playerXP[11]) + getLevelForXP(playerXP[12]) + getLevelForXP(playerXP[13]) + getLevelForXP(playerXP[14]) + getLevelForXP(playerXP[15]) + getLevelForXP(playerXP[16]) + getLevelForXP(playerXP[17]) + getLevelForXP(playerXP[18]) + getLevelForXP(playerXP[19]) + getLevelForXP(playerXP[20]));;
	...
	/*	if(System.currentTimeMillis() - saveGameDelay > Config.SAVE_TIMER && !disconnected) {
				...
				c2.sendMessage("<shad=132833> Auto Saver: Game saved succesfully for everyone.");
			//}
